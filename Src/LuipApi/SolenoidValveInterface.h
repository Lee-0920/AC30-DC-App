...
/**
 * @brief 查询当前传感器映射图。
 * @return 通道映射图，Uint8，每位表示一个通道的开关状态，1为打开(有水)，0为闭合(无水)，低位开始。
 * bit0：开关门传感器；bit1：蠕动泵进水管液位传感器；bit2：注射器进水管液位传感器；bit3：漏液传感器1；bit4：漏液传感器2
 */
#define DSCP_CMD_SVI_GET_LIQUID_SENSOR_MAP          (DSCP_SVI_CBASE + 0x03)

/**
 * @brief 查询当前温度。
 * @return float temperature
 */
#define DSCP_CMD_SVI_GET_CURRENT_TEMPERATURE         (DSCP_SVI_CBASE + 0x04)
